      ...
      <div className="relative flex min-h-[100svh] items-center px-5 py-10 sm:px-8">
        <div className="mx-auto grid w-full max-w-6xl gap-10 lg:grid-cols-[1.1fr_0.9fr] lg:items-end">
          ...
          <div className="w-full max-w-md border border-white/20 bg-white/10 p-6 shadow-2xl backdrop-blur-md sm:p-8 lg:ml-auto">
            <p className="text-sm font-semibold uppercase tracking-[0.16em] text-[#E7D39C]">Contact</p>
            <div className="mt-5 space-y-4 text-lg">
              {email && (
                <a className="block break-words text-white underline decoration-white/35 underline-offset-4 hover:text-[#F3E8C8]" href={`mailto:${email}`}>
                  {email}
                </a>
              )}
              {phone && (
                <a className="block text-white underline decoration-white/35 underline-offset-4 hover:text-[#F3E8C8]" href={`tel:${phone}`}>
                  {phone}
                </a>
              )}
